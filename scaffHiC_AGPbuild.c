...
static int *ctg_rcdex,*ctg_order,*ctg_list,*ctg_head;
...
//        printf("fgets command error:\n);
    ...
    if((ctg_order= (int *)calloc(i_contig,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - ctg_order\n");
      exit(1);
    }
    ...
    while(fscanf(namef,"%s %d %d %d %d %d",tmptext,&ctg_sfdex[i],&ctg_index[i],&ctg_length[i],&ctg_rcdex[i],&ctg_order[i])!=EOF)
    {
      st = strrchr(tmptext,':');
...
/*  Output AGP file   */
    if((fpOutfast2 = fopen(argv[args+1],"w")) == NULL)
    {
    ...
    for(i=0;i<num_hit;i++)
    {
       ...
       if((ctg_order[idd]%2) == 0)
       {
         for(j=0;j<ctg_list[i];j++)
         {
            int idk = ctg_head[i]+j;
            fprintf(fpOutfast2,"contigg1:%08d %d %d %d %d\n",ctg_mscore[idk],ctg_sfdex[idk],ctg_index[idk],ctg_length[idk],ctg_rcdex[idk]);
         }
       }
       else
       {
         for(j=(ctg_list[i]-1);j>=0;j--)
         {
            int idk = ctg_head[i]+j;
            fprintf(fpOutfast2,"contigg1:%08d %d %d %d %d\n",ctg_mscore[idk],ctg_sfdex[idk],ctg_index[idk],ctg_length[idk],ctg_rcdex[idk]);
         }
...
/*
    for(i=0;i<nSeq;i++)
    {
       ...
       if(ctg_mask[i] == 0)
       {
         fprintf(fpOutfast2,"scaffhic_%d %d %d %d %d %d %d\n",n_scaff,i,seqp->length,0,0,Max_Gap,6000);
         n_scaff++;
       }
    }    */
