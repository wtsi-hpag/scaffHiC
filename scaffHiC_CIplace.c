...
static int *ctg_length,*hit_index,*hit_ctgdex,*hit_offset;
static int *scf_offset,*scf_length,*ctg_index,*ctg_scaffs,*ctg_offset,*ctg_mscore;
static int *scf_index,*ctg_gaplen,*hit_tscore,*ctg_rcdex,*ctg_list,*ctg_head;
...
int main(int argc, char **argv)
{
    ...
    int ctg_numbs,nContigs,n_reads,nseq,iscaf1,idd1,offset1;
    int stopflag;
    char *st,*ed;
...
/*  read the alignment files         */
    i=0;
    max_ctg = 0;
    while(fscanf(namef,"%s %d %s %d %d",R_Name[i],&hit_tscore[i],rdname,&hit_locus1[i],&hit_score[i])!=EOF)
    {
        st = rdname;
        ed = strrchr(rdname,'_');
        idt = atoi(ed+1);
    ...
    if((ctg_index = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_index\n");
      exit(1);
    }
    if((scf_index = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - scf_index\n");
      exit(1);
    }
    if((scf_length = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - scf_index\n");
      exit(1);
    }
    if((ctg_offset = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_offset\n");
      exit(1);
    }
    if((ctg_length = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_offset\n");
      exit(1);
    }
    if((ctg_gaplen = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_gaplen\n");
      exit(1);
    }
    if((ctg_rcdex = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_rcdex\n");
      exit(1);
    }
    if((ctg_mscore = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_mscore\n");
      exit(1);
    }
    if((ctg_scaffs = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_scaffs\n");
      exit(1);
    }
    if((scf_offset = (int *)calloc(ctg_numbs,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_scaffs\n");
      exit(1);
    }
...
/*  read the alignment files         */
    ...
    while(fscanf(namef,"%s %d %d %d %d %d %d",rdname,&ctg_index[i],&ctg_length[i],&ctg_offset[i],&ctg_gaplen[i],&rcdex,&ctg_mscore[i])!=EOF)
    {
        ...
        scf_length[ctg_index[i]] = ctg_length[i];
...
//       printf("%s tarseq_%d %d %d %d %d %d %d\n",R_Name[i],iscaf,offset,hit_score[i],hit_tscore[i],scf_length[iscaf]-Gap_len,hit_index[i],iscaf);
         if(iscaf1 == iscaf)
         {
           if(ctg_rcdex[idd] == 0)
             ...
             offset = scf_length[idd] - hit_locus1[i]+scf_offset[idd]-read_len; 
           if(ctg_rcdex[idd1] == 0)
             offset1 = hit_locus1[i+1]+scf_offset[idd1]; 
           else
             offset1 = scf_length[idd1] - hit_locus1[i+1]+scf_offset[idd1]-read_len;
...
//           printf("%s tarseq_%d %d %d %d %d %d %d\n",R_Name[i],iscaf,idd,offset,hit_score[i],scf_length[idd],scf_offset[idd],hit_locus1[i]);
//           printf("%s tarseq_%d %d %d %d %d %d %d\n",R_Name[i],iscaf,idd1,offset1,hit_score[i],scf_length[idd1],scf_offset[idd1],hit_locus1[i+1]);
           fprintf(namef,"%s %d tarseq_%d %d %d\n",R_Name[i],hit_tscore[i],iscaf,offset,hit_score[i]);
           fprintf(namef,"%s %d tarseq_%d %d %d\n",R_Name[i+1],hit_tscore[i+1],iscaf1,offset1,hit_score[i+1]);
         }
       }
       else
       {
