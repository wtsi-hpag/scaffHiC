...
static int *break_index,*break_locus,*ctg_covermin,*ctg_coverave;
...
//        printf("fgets command error:\n);
    ...
    if((ctg_covermin = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - gap_locus2\n");
      exit(1);
    }
    if((ctg_coverave = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - gap_locus2\n");
      exit(1);
    }
...
/*  read the break file         */
    i=0;
    while(fscanf(namef,"%s %d %d %d %d %s %d",readname,&break_index[i],&break_locus[i],&ctg_length[i],&ctg_covermin[i],tempseq,&ctg_coverave[i])!=EOF)
    {
...
/*   read the alignment files         */
     ...
     {
        fprintf(namef,"Break: %d %d %d %d\n",break_index[i],break_locus[i],ctg_length[i],nContigs);
     }
     fclose(namef);
}
