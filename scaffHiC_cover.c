...
static int *hit_locus1,*hit_index,*hit_locus2,*hit_tagcode,*ctg_index,*ctg_length;

/* SSAS default parameters   */
static int IMOD=0;
static int insert_len=2000;
...
/*
    ...
    {
      printf("fmate: calloc - hit_locus12\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %d %d",readname,S_Name[i],&hit_locus1[i],&hit_locus2[i],&ctg_length[i])!=EOF)
    {
        int idt;
        st = S_Name[i];
...
//    printf("%s %s %d %d %d %d\n",S_Name[i],readname,hit_locus1[i],hit_tagcode[i],ctg_index[i],hit_locus2[i]);
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     if((cov_genome= (int *)calloc(max_len+1000,sizeof(int))) == NULL)
     {
     ...
     if((hit_map2= (int *)calloc(nSeq,sizeof(int))) == NULL)
     {
       printf("ERROR Memory_Allocate: calloc - hit_maps\n");
       exit(1);
     }
     ...
     for(i=0;i<(nSeq-1);i++)
     {
        ...
        for(k=i;k<j;k++)
        {
           int offset = hit_locus1[k];
           offset = hit_locus1[k]-(hit_locus1[k]%100)+100;
           for(n=offset;n<=hit_locus2[k];)
           {
//              if((hit_locus2[k]-hit_locus1[k]) < insert_len)
//        printf("offset: %s %d %d\n",S_Name[i],n,offset); 
              cov_genome[n]++;
              n = n + 100;
           }
...
//        printf("Scaff: %s\n",S_Name[i]); 
        ...
        {
           if((n%100) == 0)
             printf("%s %d %d\n",S_Name[i],n,cov_genome[n]); 
        }
//               fprintf(namef,"%s tarseq_%d %d %d %d\n",S_Name[n],ctg_index[n],offset1,offset2,ctg_length[n]); 
        i = j - 1;
     }
     fclose(namef);
}
