...
/*  Output AGP file   */
    ...
    for(i=0;i<num_hit;i++)
    {
       ...
       for(j=0;j<ctg_list[i];j++)
       {
          ...
          i_contig++;
//            fprintf(fpOutfast2,"Scaffold_%d\t%d\t%d\t%d\t%c\t%s\t%d\t%d\t%c\n",n_scaff,t_len,t_len+ctg_length[idk],j+1,'W',outName,1,ctg_length[idk],'+');
//          fprintf(fpOutfast2,"Scaffold_:%08d\t%d\t %d %d %d\n",n_scaff,ctg_mscore[idk],ctg_sfdex[idk],ctg_index[idk],ctg_length[idk],ctg_rcdex[idk]);
       }
       n_scaff++;
    }
