...
/*
     ...
     for(i=0;i<nSeq;i++)
     {
        ...
        if(num_hits>=10) 
        {
          ...
          {
            float rate2 = 0.05;
            rate2 = sqrt(rate2/rate)*(rate2/rate)*20000;
          ...
          for(k=0;k<n_cells;k++)
          {
             ...
             if(n_file == 0)
             {
//              if((heat_map[k][k-1] < 10)&&(sum_gap < 2)&&(k>5)&&(k<(n_cells-5)))
//              if((sum_gap < 2)&&(k>5)&&(k<(n_cells-5)))
              {
                int sum_belt = 0;
                for(n=k;n<(k+10)&&(n<n_cells)&&(k>=belt_size);n++)
                   sum_belt = sum_belt + heat_map[n][k-belt_size];
                printf("Map: %d %d %d %d |%d| ",hit_index[i],k,k*n_lattice,sum_gap,sum_belt);
                for(n=0;n<n_cells;n++)
                {
                  if((n!=k)&&(n<k)&&(n>(k-belt_size)))
                     printf(" %d ",heat_map[k][n]);
                }
                printf("\n");
              }
             }
