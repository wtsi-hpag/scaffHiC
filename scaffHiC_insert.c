...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s <tarseq.tag> <align.dat> <align.size>\n",argv[0]);
     ...
     for(i=0;i<(nSeq-1);i++)
     {
	  ...
	  kk = j-i;
          ...
          for(n=i;n<j;n++)
          {
             ...
             if((num_hit2==2))
             {
               int insert_size = abs(hit_locus[n]-hit_locus[n+1]);
                  k = n;
//                  printf("%s tarseq_%d %d %d %d\n",S_Name[k],ctg_index[k],hit_locus[k],hit_score[k],hit_tagcode[k]); 
//                  fprintf(namef,"%s tarseq_%d %d %d %d %d\n",S_Name[k],ctg_index[k],hit_locus[k],hit_score[k],hit_tagcode[k],ctg_length[ctg_index[k]]); 
                  fprintf(namef,"size: %d\n",insert_size); 
             }
             n=m-1;
          }
