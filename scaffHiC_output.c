...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_scaffold_fastq> <HiC-structure_file> <output_scaffold_fastq> <output_scaffold_agp>\n",argv[0]);
      exit(1);
    }
...
//       printf("@%s\n",seqp->name);
       ...
       for(j=0;j<ctg_list[i];j++)
       {
          ...
          fprintf(fpOutfast2,"scaffHiC_%d %d %d %d %d %d %d\n",n_scaff,ctg_index[idd],seqp->length,sup_len,Max_Gap,ctg_rcdex[ctg_index[idd]],ctg_mscore[ctg_index[idd+1]]);
          if(j<(ctg_list[i]-1))
          {
...
//         fprintf(fpOutfast,"@%s\n",seqp->name);
         ...
         for(rc=0;rc<seq_ed;rc++)
             putc(40+041,fpOutfast);
         fprintf(fpOutfast,"\n");
         fprintf(fpOutfast2,"scaffHiC_%d %d %d %d %d %d %d\n",n_scaff,i,seqp->length,0,0,Max_Gap,0);
         n_scaff++;
       }
    }
    fclose(fpOutfast);
