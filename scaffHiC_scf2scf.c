...
static char **S_Name;
static int *ctg_index,*ctg_rcdex,*ctg_list,*ctg_head,*ctg_id2id,*ctg_ordex;
...
//        printf("fgets command error:\n);
    ...
    if((ctg_scflen= (int *)calloc(i_contig,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - ctg_scflen\n");
      exit(1);
    }
    if((ctg_ordex= (int *)calloc(i_contig,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - ctg_ordex\n");
      exit(1);
    }
    ...
    while(fscanf(namef,"%s %d %d %d %d %d %d",tmptext,&ctg_index[i],&ctg_length[i],&ctg_scflen[i],&ctg_gaplen[i],&ctg_rcdex[i],&ctg_mscore[i])!=EOF)
    {
      st = strrchr(tmptext,'_');
      ctg_sfdex[i] = atoi(st+1);
      ctg_id2id[i] = ctg_index[i];
      if(run_flag == 1)
        ctg_ordex[i] = ctg_index[i];
      else
        ctg_ordex[i] = ctg_scflen[i];
      i++;
    }
    fclose(namef);
...
//        printf("fgets command error:\n);
    ...
    if((ctg_ordex2= (int *)calloc(i_contig,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - ctg_scflen\n");
      exit(1);
    }
    ...
    while(fscanf(namef,"%s %d %d %d %d %d %d",tmptext,&ctg_index2[i],&ctg_length2[i],&ctg_ordex2[i],&ctg_gaplen2[i],&ctg_rcdex2[i],&ctg_mscore2[i])!=EOF)
    {
      st = strrchr(tmptext,'_');
