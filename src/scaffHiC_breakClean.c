...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_break_file> <output_break_file>\n",argv[0]);
...
/*  read the break file         */
    i=0;
    while(fscanf(namef,"%s %d %d %d %s %s %d %s %d",readname,&break_index[i],&break_locus[i],&ctg_length[i],tempseq,tempseq,&ctg_covermin[i],tempseq,&ctg_coverave[i])!=EOF)
    {
