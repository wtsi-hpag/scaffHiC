...
static char **F_Name,**R_Name,**R_Name2,**cigarline,**cellname;
static int *read1_index,*read2_index;
...
int main(int argc, char **argv)
{
    ...
    int i,j,k,nSeq,args,num_align,stopflag,offset,n_hits,num_fs,num_rs;
    ...
    if((read1_index = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_left\n");
      exit(1);
    } 
    if((read2_index = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_left\n");
      exit(1);
    } 
...
/*  read the alignment files         */
    ...
    if(num_fs != num_rs)
    {
      printf("ERROR: read 1 number is not equal to read 2 number!\n");
      exit(1);
    }
    nseq=0;
...
/*   read the cigar line file   */
    ...
    while(!feof(namef))
    {
      ...
      if(((num_align)%4)==0)
      {
        long idd = num_align/4;;
        fprintf(namef2,"@HiC-reads_%010ld_F%d_R%d\n",idd,read1_index[idd],read2_index[idd]);
      }
      else
      {
        if(((num_align)%4)!=0)
