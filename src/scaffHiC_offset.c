...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %d %d %d",S_Name[i],readname,&hit_locus[i],&hit_score[i],&hit_tagcode[i],&ctg_length[i])!=EOF)
    {
        int idt;
        st = readname;
	  ...
	  kk = j-i;
          ...
          for(n=i;n<j;n++)
          {
             ...
             if(num_hit2==2)
             {
               int offset1 = 0;
               int offset2 = 0;
               if(hit_locus[n] > hit_locus[n+1])
               {
                 offset1 = hit_locus[n+1];
                 offset2 = hit_locus[n];
               }
               else
               {
                 offset1 = hit_locus[n];
                 offset2 = hit_locus[n+1];
               }
//               for(k=n;k<m;k++)
