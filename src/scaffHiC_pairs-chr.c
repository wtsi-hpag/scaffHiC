...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s -ctg1 x -ctg2 y <Input_Mapping_file> <Output_file1> <Output_file2>\n",argv[0]);
...
/*  read the alignment files         */
    i=0;
    max_ctg = 0;
    while(fscanf(namef,"%s %d %s %d %s",R_Name[i],&hit_score[i],rdname,&hit_locus1[i],tempc1)!=EOF)
    {
        st = rdname;
        ed = strrchr(rdname,'_');
        idt = atoi(ed+1);
...
/* =============================== */
{
     ...
     for(i=0;i<nSeq;i++)
     {
        ...
        if((j-i)>=2) 
        {
          ...
          {
            if((hit_index[i] == i_ctg1)&&(hit_index[i+1] == i_ctg1))
            {
              ctg_idex1[i] = 1;
              ctg_idex2[i+1] = 1;
            }
...
//          printf("www: %s %d\n",R_Name[i],superlength[i]);
     ...
     for(i=0;i<nSeq;i++)
     {
        if(ctg_idex1[i] == 1)
        {
          fprintf(namef,"%s %d %d %d\n",R_Name[i],hit_locus1[i],hit_locus1[i+1],hit_locus1[i]-hit_locus1[i+1]);
//          fprintf(namef,"%s %d %d %d\n",R_Name[i],hit_locus1[i],hit_locus1[i+1],55358864-hit_locus1[i]);
//          fprintf(namef,"%s %d\n",R_Name[i+1],hit_locus1[i+1]);
          i++;
        }
     }
     fclose(namef);
