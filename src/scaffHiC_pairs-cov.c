...
/*  read the alignment files         */
    i=0;
    max_ctg = 0;
    while(fscanf(namef,"%s %d %d %d",R_Name[i],&hit_locus1[i],&hit_locus2[i],&idt)!=EOF)
    {
        ctg_length[i] = abs(idt);
        if(idt > 0)
        {
          if(hit_locus1[i] > max_ctg)
            max_ctg = hit_locus1[i];
        }
        else if(idt < 0)
        {
...
/* =============================== */
{
     ...
     if((cover_genome = (int *)calloc(rsize,sizeof(int))) == NULL)
     {
     ...
     {
       int st,ed;
       if(ctg_length[i] < 500000)
       {
        if(hit_locus1[i] > hit_locus2[i])
        {
           st = hit_locus2[i];
           ed = hit_locus1[i];
        }
        else if(hit_locus2[i] > hit_locus1[i])
        {
           st = hit_locus1[i];
           ed = hit_locus2[i];
        }
