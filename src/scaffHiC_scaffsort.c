...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s [-block 2500] [-reads 10] [-score 20] <input_barcode_file> <barcode_sorted_file>\n",argv[0]);
...
//    while(fscanf(namef,"%s %s %s %s %s %s",tempc1,readname,lociname,tempc1,tempc1,tempc1)!=EOF)
    {
        ...
        else
        {
          ...
          readIndex[j] = j;
    printf("%d %s %s %s %d %d\n",j,S_Name[j],lociname,readname,idi,idj);
        }
        i++;
    }
    fclose(namef);
...
//             printf("Duplicates: %d %d %s %s\n",k,readIndex[n],S_Name[i],R_Name[idd]);
             hit_masks[idd] = 1;
             hit_masks[idd+1] = 1;
          }
        }
